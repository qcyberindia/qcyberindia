...
export default function CategorySection({ category, index }: { category: ServiceCategory; index: number }) {
  return (
    <section id={category.id} className="py-14 border-t border-[var(--color-line)] first:border-t-0 first:pt-0">
      <div className="flex items-baseline gap-4">
        <span className="font-display text-2xl font-bold text-[var(--color-gold)]">
          {String(index + 1).padStart(2, "0")}
        </span>
        <div>
          <h2 className="font-display text-2xl font-bold text-[var(--color-navy)]">{category.title}</h2>
